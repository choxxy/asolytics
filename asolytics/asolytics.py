...
parser.add_argument('--average', dest='average', type=str,
                    help='Average number of downloads / ratings per day (--average org.telegram.messenger)')

parser.add_argument('--tracker', dest='tracker', type=str,
                    help='Track the position of the application in the search (--tracker "quizzes; zombie games; online shooters")')
...
parser.add_argument('--file', dest='file', action='store_true',
                    help='Use this key if the --tracker option points to a keyword file (each key must be on a newline)')
parser.add_argument('--extract', dest='extract', type=str,
                    help='Identify keywords used in application metadata. The title, developer name, short description, full description, reviews are analyzed. (--extract org.telegram.messenger)')
parser.add_argument('--similar', dest='similar', type=str,
                    help='Analysis of similar applications. Use this key to analyze the pages of your competitors where you appear in similar ones (--similar org.telegram.messenger)')
parser.add_argument('--local', dest='local', type=str,
                    help='Analysis of naming localization. Languages into which the Google Play application page is translated (--local org.thoughtcrime.securesms)')
parser.add_argument('--reviews', dest='reviews', type=str,
                    help='Analyze recorded reviews in different locales. Reviews that are in the top (--reviews org.thoughtcrime.securesms)')
parser.add_argument('--csv', dest='csv', type=str,
                    help='Use this option to save the result to a file, csv can be opened with spreadsheets like Excel (--csv file.csv)')
parser.add_argument('--tags', dest='tags', type=str,
                    help='Tag analysis tool in Google Play, checking the indexing of the application by tags (--tags org.thoughtcrime.securesms)')
...
def tags_analysis(bundleId):
    ...
    x = PrettyTable()
    x.field_names = ["Tag", "The type tag", "The key tag", "Significance for the cluster", "Number of applications",
                     "Indexing by tag"]
...
def action_parser_similar_app(bundleId):
    ...
    x = PrettyTable()
    x.field_names = ["The name of the application / Bundle ID", "Your position is similar", "Number of downloads",
                     "Downloads per day"]
    ...
    if (len(pos) > 0):
        ...
        print("Aaverage number of installs to applications on the pages of which you are offended: from " + str(
            int(sum(ins_x1) / len(ins_x1))) + " to " + str(int(sum(ins_x2) / len(ins_x2))))
        print(
            "The average number of installs to data per tobu, on the pages of which you from are offended: from " + str(
                int(sum(ins_x1_daily) / len(ins_x1_daily))) + " to " + str(int(sum(ins_x2_daily) / len(ins_x2_daily))))
    else:
...
def tracker_position_google_play():
    ...
    if count_days_in_history > 0:
        x.field_names = ["Key phrase", "Search position", "Position yesterday", "Has changed", "Entries in names",
                         "Login in the name of the developer"]
