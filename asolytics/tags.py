...
class Google_play_tags():
    ...
    def get_similar_links(self, browser: webdriver.Firefox, bundleID: str, gl: str) -> List[str]:
        ...
        try:
            ...
            button = self.find_element_by_attribute_value(elements, "aria-label",
                                                          "See more information on Similar apps")
            if button is None:
                button = self.find_element_by_attribute_value(elements, "aria-label",
                                                              "See more information on Similar games")
            if button is not None:
                ...
                for el in elements:
                    ...
                    if (not links.__contains__(href)) and (
                            href is not None and href.startswith("https://play.google.com/store/apps/details?id=")):
                        links.append(href)
        except Exception as e:
            print(e.args)
        return links

    def find_element_by_attribute_value(self, elements: List[WebElement], attribute_name: str,
                                        attribute_value: str) -> WebElement:
        for el in elements:
            attr = el.get_attribute(attribute_name)
            if attr is not None and attr.lower() == attribute_value.lower():
                return el
        return None
